---
title: Semantic Layer Tools
type: landscape
---

## Commercial / Proprietary

- [dbt Semantic Layer](https://www.getdbt.com/product/semantic-layer) (dbt Labs)
  - Hosted commercial layer via dbt Cloud (see Open Source)
- [AtScale](https://www.atscale.com/)
- [Cube](https://cube.dev/)
  - Hosted commercial layer under the same brand as the open-source core (see Open Source)

## Open Source / Provider-agnostic

- [dbt Semantic Layer](https://github.com/dbt-labs/metricflow) (dbt Labs)
  - Self-hostable core via dbt Core and MetricFlow, with a commercial layer via dbt Cloud (see Commercial)
- [Cube](https://github.com/cube-js/cube)
  - Self-hostable core, with a commercial layer under the same brand (see Commercial)

## Legacy / Decommissioned

No legacy or decommissioned semantic layer tools are catalogued in this category.
