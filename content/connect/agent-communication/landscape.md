---
title: Communication and Interoperability Landscape
type: landscape
---

## Commercial / Proprietary

No commercial protocols are catalogued in this category.

## Open Source / Provider-agnostic

- [A2A](https://a2a-protocol.org/) (Google)
  - An open source project under the Linux Foundation, contributed by Google (Apache 2.0)
- [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) (Zed)
  - Lets external agents run as first-class citizens inside a host editor
  - Example integrations: Codex, Claude Agent, OpenCode
  - [Created by Zed](https://zed.dev/acp), which is also a notable host editor implementation (see [[build/agent-harnesses/landscape|Harnesses Landscape]])
  - Not the same protocol as IBM's ACP (see Legacy)

## Legacy / Decommissioned

- [Agent Communication Protocol (ACP)](https://github.com/i-am-bee/acp) (IBM)
  - Merged into A2A under the Linux Foundation, [announced August 25, 2025](https://github.com/orgs/i-am-bee/discussions/5). It no longer exists as a standalone protocol, and its repository is archived
  - Not to be confused with the Agent Client Protocol above, which shares the acronym but is unrelated and still active
