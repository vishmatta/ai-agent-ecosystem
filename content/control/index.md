...
A handful of concerns do not attach cleanly to a single stage even after that split, so their content stays here: data governance, since data is handled at connectors, at context assembly, and in logs alike; agent identity, issued at deploy time but presented and audited elsewhere; audit logs and human oversight; the Policies hub, covering how policies are authored and enforced regardless of subject; and [[control/observability-and-evaluation/index|observability, evaluation, and debugging together]], since they share the same tooling.
